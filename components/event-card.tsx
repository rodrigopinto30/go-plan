...
const EventCard: React.FC<EventCardProps> = ({
  ...
  if (variant === "list") {
    return (
      <Card
        className={`py-0 group cursor-pointer hover:shadow-lg transaction-all hover:border-purple-500/50 ${className}`}
        onClick={() => onClick}
      >
        <CardContent className="p-3 flex gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-sm mb-1 group-hover:text-purple-400 trnasition-colors line-clamp-2">
              {event.title}
            </h3>

            <p className="text-xs text-muted-foreground mb-1">
              {format(event.startDate, "EEE, dd MM, HH:mm")}
            </p>
